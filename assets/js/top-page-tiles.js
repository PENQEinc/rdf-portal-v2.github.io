...
class TopPageTilingDatasetsViewController {
  constructor() {
    ...
    console.log(
      "TopPageTilingDatasetsView: container found?",
      !!this.container
    );
  ...
  async init() {
    ...
    try {
      ...
      console.log(
        `データセット情報を読み込み完了: ${this.datasetsData.length}件`
      );
    } catch (error) {
      this.hideLoadingState();
      console.warn(
        "データセット情報の読み込みに失敗しました。静的なタイルを表示します。",
        error
      );
      this.createStaticTiles();
  ...
  showLoadingState() {
    this.container.innerHTML =
      '<div class="loading-message">データセット情報を読み込み中...</div>';
  }
  ...
  createDynamicTiles() {
    ...
    const datasetsWithMetadata = this.datasetsData.filter(
      (dataset) => dataset.title && dataset.description
    );

    // メタデータがないデータセットも一部含める
    const datasetsWithoutMetadata = this.datasetsData.filter(
      (dataset) => !dataset.title || !dataset.description
    );
  ...
  createTile(data, index, isDynamic = false) {
    ...
    if (isDynamic) {
      ...
      tile.addEventListener("mouseenter", () =>
        this.handleTileHover(tile, true)
      );
      tile.addEventListener("mouseleave", () =>
        this.handleTileHover(tile, false)
      );
    }
    ...
    const margin = 20;
    const tileWidth =
      window.innerWidth > 768 ? 130 : window.innerWidth > 480 ? 90 : 75;
    const tileHeight =
      window.innerWidth > 768 ? 90 : window.innerWidth > 480 ? 70 : 55;
  ...
  handleTileHover(tile, isHovering) {
    ...
    if (isHovering) {
      ...
      setTimeout(() => {
        ...
        siblings.forEach((sibling) => {
          if (sibling !== tile) {
            sibling.style.transform = sibling.style.transform.replace(
              /translate\([^)]*\)/g,
              ""
            );
          }
  ...
  enhanceAccessibility() {
    const tiles = this.container.querySelectorAll(
      '.dataset-tile[style*="cursor: pointer"]'
    );
    ...
    tiles.forEach((tile, index) => {
      ...
      tile.setAttribute(
        "aria-label",
        `データセット: ${tile.querySelector(".title").textContent}`
      );
